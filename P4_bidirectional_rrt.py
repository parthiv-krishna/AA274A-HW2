...
class RRTConnect(object):
    ...
    def reconstruct_path(self, n_fw, n_bw, V_fw, V_bw, P_fw, P_bw):
        """
        Helper function to reconstruct the path generated by the bidirectional RRT.

        Inputs:
            n_fw: current index in the forward tree
            n_bw: current index in the backward tree
            V_fw: array representing vertices in the forward tree
            V_bw: array representing vertices in the backward tree
            P_fw: array representing edges in the forward tree
            P_bw: array representing edges in the backward tree
        Output:
            None, but updates self.path to contain a list of states
                leading from x_init to x_goal.
        """

        self.path = []
        # construct path from meeting point to start
        idx = n_fw
        while idx != -1: # parent of root is -1
            self.path.insert(0, V_fw[idx,:])
            idx = P_fw[idx] # go to parent of this node
 
        # construct path from meeting point to end
        idx = P_bw[n_bw] # skip straight to parent to avoid duplicating meeting point
        while idx != -1:
            self.path.append(V_bw[idx,:])
            idx = P_bw[idx]
    ...
    def solve(self, eps, max_iters = 1000):
        ...
        for k in range(max_iters):
            ...
            if len(self.x_init) == 2:
                x_rand = np.array([randomx, randomy])
            else: # x, y, th (dubins) 
                randomth = np.random.uniform(0, np.pi)
                x_rand = np.array([randomx, randomy, randomth])
            
            # attach x_new, a point in the direction of x_rand, to forward tree
            x_near_idx_fw = self.find_nearest_forward(V_fw[0:n_fw,:], x_rand)
            x_near = V_fw[x_near_idx_fw,:]
            x_new = self.steer_towards_forward(x_near, x_rand, eps)
        
            if self.is_free_motion(self.obstacles, x_near, x_new):
                # add new point and edge to forward tree
                V_fw[n_fw,:] = x_new
                P_fw[n_fw] = x_near_idx_fw

                # find nearest point in backward tree
                x_near_idx_bw = self.find_nearest_backward(V_bw[0:n_bw,:], x_new)
                x_connect = V_bw[x_near_idx_bw,:]

                while True:
                    x_newconnect = self.steer_towards_backward(x_new, x_connect, eps)
                    if self.is_free_motion(self.obstacles, x_newconnect, x_connect):
                        # add new point and edge to backward tree
                        V_bw[n_bw,:] = x_newconnect
                        P_bw[n_bw] = x_near_idx_bw
                        if np.array_equal(x_newconnect, x_new):
                            self.reconstruct_path(n_fw, n_bw, V_fw, V_bw, P_fw, P_bw)
                            success = True
                            break
            ...
            if success: # terminate for loop early
                break
            ...
            # attach x_new, a point in the direction of x_rand, to backward tree
            x_near_idx_bw = self.find_nearest_backward(V_bw[0:n_bw,:], x_rand)
            x_near = V_bw[x_near_idx_bw,:]
            ...
            if self.is_free_motion(self.obstacles, x_near, x_new):
                ...
                # find nearest point in forward tree
                x_near_idx_fw = self.find_nearest_forward(V_fw[0:n_fw,:], x_new)
                x_connect = V_fw[x_near_idx_fw,:]
...
class DubinsRRTConnect(RRTConnect):
    ...
    def steer_towards_backward(self, x1, x2, eps):
        ...
        point_rev = x1_rev if self.distance(x2_rev, x1_rev) < eps else np.array(path_sample(x2_rev, x1_rev, 1.001*self.turning_radius, eps)[0][1])
